# -*- coding: utf-8 -*-
"""
Created on Fri Mar  6 20:21:26 2020

@author: 1015299
"""
#This is the simulation code for the Continuous wave case
#imports 
...
from Cont_wave_fieldconfig import E_field 
from Cont_wave_fieldconfig import B_field
from Cont_wave_fieldconfig import a_field
...
dt = 2*(np.pi)/(part*w)
n = 1
iter_num = int(n*pulse_length/dt)
num_pos = 50
...
#field functions
def e_field(z, t):
    e1 = E_field(z, t, k, w, E0, 0, 0)
    e2 = E_field(z, t, k, w, E0, -(np.pi/2), (np.pi/2))
    e3 = ratio*E_field(z, t, -k, w, E0, 0, 0)
    e4 = ratio*E_field(z, t, -k, w, E0, -(np.pi/2), (np.pi/2))
    E1 = (np.sqrt(0.5))*(e1 + e2 + e3 + e4)
    return E1

def b_field(z, t):
    b1 = B_field(z, t, k, w, E0, 0, 0)
    b2 = B_field(z, t, k, w, E0, -(np.pi/2), (np.pi/2))
    b3 = ratio*B_field(z, t, -k, w, E0, 0, 0)
    b4 = ratio*B_field(z, t, -k, w, E0, -(np.pi/2), (np.pi/2))
    E1 = (np.sqrt(0.5))*(b1 + b2 + b3 + b4)
    return E1

def A_field(z, t):
    A1 = a_field(z, t, k, w, E0, 0, 0)
    A2 = a_field(z, t, k, w, E0, -(np.pi/2), (np.pi/2))
    A3 = ratio*a_field(z, t, -k, w, E0, 0, 0)
    A4 = ratio*a_field(z, t, -k, w, E0, -(np.pi/2), (np.pi/2))
...
for j in range(num_pos):
    ...
    l = 0
    l1 = 0
    Ein = e_field(z[0], (-1)*part/(2*dt))
    Bin = b_field(z[0], (-1)*part/(2*dt))
    ...
    for i in range(iter_num):
        ...
        y.append(r_new[1])
        z.append(r_new[2])
        E2 = e_field(z[i+1], i*dt)
        B2 = b_field(z[i+1], i*dt)
...
plt.savefig('tz_rc_'+str(rc)+'_rat_'+str(ratio)+'_final_norm.pdf')
...
plt.savefig('pxpy_rc_'+str(rc)+'_rat_'+str(ratio)+'_final_norm.pdf')
...
plt.savefig('zpz_rc_'+str(rc)+'_rat_'+str(ratio)+'_final_norm.pdf')
...
plt.xlabel('$t/s$', fontsize = 16)
plt.ylabel('$p_z$/$m_e c$', fontsize = 16)
plt.savefig('tpz_rc_'+str(rc)+'_rat_'+str(ratio)+'_final_norm.pdf')
...
#plt.savefig('tgamma_rc_'+str(rc)+'_rat_'+str(ratio)+'_final.pdf')
...
plt.xlabel('$z/\mu m$', fontsize = 16)
plt.ylabel('$\gamma$', fontsize = 16)
#plt.savefig('zgamma_rc_'+str(rc)+'_rat_'+str(ratio)+'_final.pdf')
...
plt.xlabel('$t/s$', fontsize = 16)
plt.ylabel('$z/\mu m$', fontsize = 16)
plt.savefig('tz_rc_'+str(rc)+'_rat_'+str(ratio)+'_detailed_final_norm.pdf')
...
plt.xlabel('$p_x$/$m_e c$', fontsize = 16)
plt.ylabel('$p_y$/$m_e c$', fontsize = 16)
plt.savefig('pxpy_rc_'+str(rc)+'_rat_'+str(ratio)+'_detailed_final_norm.pdf')
...
plt.xlabel('$z/\mu m$', fontsize = 16)
plt.ylabel('$p_z$/$m_e c$', fontsize = 16)
plt.savefig('zpz_rc_'+str(rc)+'_rat_'+str(ratio)+'_detailed_final_norm.pdf')
...
plt.xlabel('$t/s$', fontsize = 16)
plt.ylabel('$p_z$/$m_e c$', fontsize = 16)
plt.savefig('tpz_rc_'+str(rc)+'_rat_'+str(ratio)+'_detailed_final_norm.pdf')
...
plt.xlabel('$t/s$', fontsize = 16)
plt.ylabel('$\gamma$', fontsize = 16)
#plt.savefig('tgamma_rc_'+str(rc)+'_rat_'+str(ratio)+'_detailed_final.pdf')
...
plt.xlabel('$z/\mu m$', fontsize = 16)
plt.ylabel('$\gamma$', fontsize = 16)
#plt.savefig('zgamma_rc_'+str(rc)+'_rat_'+str(ratio)+'_detailed_final.pdf')
...
#plt.savefig('etat_rc_'+str(rc)+'_rat_'+str(ratio)+'_final.pdf')
...
plt.xlabel('$z/\mu m$', fontsize = 16)
plt.ylabel('$\eta$', fontsize = 16)
#plt.savefig('etaz_rc_'+str(rc)+'_rat_'+str(ratio)+'_final.pdf')
...
plt.xlabel('$t/s$', fontsize = 16)
plt.ylabel('$\eta$', fontsize = 16)
#plt.savefig('etat_rc_'+str(rc)+'_rat_'+str(ratio)+'_detailed_final.pdf')
...
plt.xlabel('$z/\mu m$', fontsize = 16)
plt.ylabel('$\eta$', fontsize = 16)
#plt.savefig('etaz_rc_'+str(rc)+'_rat_'+str(ratio)+'_detailed_final.pdf')
